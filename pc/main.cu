...
static void cmd_test(void) {
    ...
    {
        /* Z=0: obs_bool (sign comparison) — already tested implicitly.
         * Just confirm the API contract. */
        check("Z0 obs_bool(+42)=1", 1, obs_bool(42));
        check("Z0 obs_bool(-7)=0", 0, obs_bool(-7));
        check("Z0 obs_bool(0)=0", 0, obs_bool(0));

        /* Z=1: obs_and — both |val| and I_energy exceed threshold */
        ...
        /* Z=2: obs_xor — already tested above, skip */

        /* Z=3: obs_freq — resonant when n_incoming high relative to valence */
        ...
        /* Z=4: obs_corr — val and energy trend together */
    ...
    printf("--- Observer Depth Integration ---\n");
    {
        Engine eng; engine_init(&eng);
        Graph *g0 = &eng.shells[0].g;
        /* Two sources collide at destination */
        ...
        /* Equal-magnitude opposite signals → destructive collision */
        g0->nodes[sa].val = 80; g0->nodes[sb].val = -80;
        g0->nodes[sd].valence = 50;
        int32_t prev_val = g0->nodes[sd].val;
        /* Two separate edges from different sources */
        ...
        /* All 5 observers see the same collision differently */
        int z0 = obs_bool(d->val);
        int z1 = obs_and(d->val, d->I_energy, 30);
        int z2 = obs_xor(d->I_energy, d->val);
        int z3 = obs_freq(d->n_incoming, d->valence);  /* n_incoming cleared, so 0 */
        int z4 = obs_corr(d->val, prev_val, d->I_energy);

        /* Destructive collision: voltage near zero, I_energy high */
        check("zdep: I_energy > 0", 1, d->I_energy > 0 ? 1 : 0);
        /* Z0 depends on sign of residual — not deterministic, just check it runs */
        check("zdep: Z0 returns 0 or 1", 1, (z0 == 0 || z0 == 1) ? 1 : 0);
        /* Z2 should fire: energy present, voltage cancelled */
        check("zdep: Z2 XOR fires", 1, z2);
        /* Valence grew from collision (n_incoming was >= 2) */
        check("zdep: valence grew", 1, d->valence > 50 ? 1 : 0);
        (void)z1; (void)z3; (void)z4; /* These ran without crash — that's the test */
        engine_destroy(&eng);
    }
